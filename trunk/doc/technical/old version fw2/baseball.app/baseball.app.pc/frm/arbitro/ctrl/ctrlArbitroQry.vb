Namespace frm.arbitro.ctrl
    Public Class ctrlArbitroQry
        Inherits _template.consulta.ctrl.queryctrl

        Private _vista As frmArbitroQry
        Public Overrides Function BorrarRegistro(ByRef frm As repsol.forms.template.consulta.QueryForm) As Object
            Try
                _vista = frm

                Dim obj As baseball.lib.vo.Arbitro = Me.DataGridViewRowToVO(Me._vista.dgConsulta.CurrentRow)

                Dim acc As New baseball.lib.bl.arbitro.doRemove(obj)
                acc.execute()
        ...
        Public Overrides Sub ConsultaRegistros(ByRef frm As repsol.forms.template.consulta.QueryForm)
            Try
                ...
                Dim aTmp As List(Of baseball.lib.vo.Arbitro) = Nothing

                Dim acc As New baseball.lib.bl.arbitro.doGetAll
                aTmp = acc.execute
        ...
        Protected Overrides Function ListVOToDataView(ByVal listObject As Object) As System.Data.DataView
            Try
                ...
                Dim dt As New DataTable(GetType(baseball.lib.vo.Arbitro).FullName)
                dt.Columns.Add("Id", GetType(Int16))
                dt.Columns.Add("FullName", GetType(String))
                dt.Columns.Add("objVO", GetType(baseball.lib.vo.Arbitro))

                For Each obj As baseball.lib.vo.Arbitro In listObject
        ...
        Public Overrides Sub setEstiloGridRegistros(ByRef frm As repsol.forms.template.consulta.QueryForm)
            Try
                ...
                If (_vista.dgConsulta.DataSource Is Nothing) Then
                    ...
                    _vista.dgConsulta.Columns.Add("FullName", "Arbitro")
                ...
                cname = "FullName"
                _vista.dgConsulta.Columns(cname).HeaderText = "Arbitro"
