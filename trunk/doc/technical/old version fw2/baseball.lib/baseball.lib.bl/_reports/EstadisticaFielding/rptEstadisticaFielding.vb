Namespace _reports.EstadisticaFielding
    Public Class rptEstadisticaFielding
        ...
        Protected Overrides Function getEmbeddedReport() As String
            Try
                ...
                res = "baseball.lib.bl.rdlEstadisticaFielding.rdlc"
        ...
        Protected Overrides Function getDataSource() As Object
            Try
                Dim res As List(Of vo.EstadisticaFielding) = Nothing
                Dim doSel As New _estadistica.doGetAllForFielding(Me._filtro)
                res = doSel.execute

                If (res Is Nothing) Then res = New List(Of vo.EstadisticaFielding)
                Return res.ToArray
            Catch ex As Exception
                Throw ex
            End Try
        ...
        Public Overloads Overrides Sub setInformeIntoViewer(ByRef viewer As Microsoft.Reporting.WinForms.ReportViewer)
            Try
                Dim aObjVO As vo.EstadisticaFielding() = getDataSource()

                Dim rptDataSourceEstadistica As New Microsoft.Reporting.WinForms.ReportDataSource
                rptDataSourceEstadistica.Name = GetType(vo.EstadisticaFielding).FullName.Replace(".", "_")
