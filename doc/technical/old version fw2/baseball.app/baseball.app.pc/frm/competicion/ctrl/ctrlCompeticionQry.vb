Namespace frm.competicion.ctrl
    Public Class ctrlCompeticionQry
        Inherits _template.consulta.ctrl.queryctrl

        Private _vista As frmCompeticionQry
        Public Overrides Function BorrarRegistro(ByRef frm As repsol.forms.template.consulta.QueryForm) As Object
            Try
                _vista = frm

                Dim obj As baseball.lib.vo.Competicion = Me.DataGridViewRowToVO(Me._vista.dgConsulta.CurrentRow)

                Dim acc As New baseball.lib.bl.competicion.doRemove(obj)
                acc.execute()
        ...
        Public Overrides Sub ConsultaRegistros(ByRef frm As repsol.forms.template.consulta.QueryForm)
            Try
                ...
                Dim aTmp As List(Of baseball.lib.vo.Competicion) = Nothing

                Dim acc As New baseball.lib.bl.competicion.doGetAll
                aTmp = acc.execute
        ...
        Protected Overrides Function ListVOToDataView(ByVal listObject As Object) As System.Data.DataView
            Try
                ...
                Dim dt As New DataTable(GetType(baseball.lib.vo.Competicion).FullName)
                dt.Columns.Add("Id", GetType(Int16))
                dt.Columns.Add("Descripcion", GetType(String))
                dt.Columns.Add("objVO", GetType(baseball.lib.vo.Competicion))

                For Each obj As baseball.lib.vo.Competicion In listObject
